"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.getJobApplicants = exports.applyToJob = exports.getPublicJobById = exports.getPublicJobs = exports.deleteJob = exports.updateJob = exports.getJobById = exports.getAllJobs = exports.createJob = void 0;
const error_1 = require("../utils/error");
const jobService = __importStar(require("../services/job.service"));
exports.createJob = (0, error_1.catchAsync)(async (req, res) => {
    const data = await jobService.createJob(req.body);
    res.status(201).json({ status: 'success', data });
});
exports.getAllJobs = (0, error_1.catchAsync)(async (req, res) => {
    const data = await jobService.getAllJobs(req.query);
    res.json({ status: 'success', data });
});
exports.getJobById = (0, error_1.catchAsync)(async (req, res) => {
    const data = await jobService.getJobById(req.params.id);
    res.json({ status: 'success', data });
});
exports.updateJob = (0, error_1.catchAsync)(async (req, res) => {
    const data = await jobService.updateJob(req.params.id, req.body);
    res.json({ status: 'success', data });
});
exports.deleteJob = (0, error_1.catchAsync)(async (req, res) => {
    await jobService.deleteJob(req.params.id);
    res.status(204).json({ status: 'success', data: null });
});
exports.getPublicJobs = (0, error_1.catchAsync)(async (req, res) => {
    const data = await jobService.getPublicJobs();
    res.json({ status: 'success', data });
});
exports.getPublicJobById = (0, error_1.catchAsync)(async (req, res) => {
    const data = await jobService.getPublicJobById(req.params.id);
    res.json({ status: 'success', data });
});
exports.applyToJob = (0, error_1.catchAsync)(async (req, res) => {
    const data = await jobService.applyToJob(req.params.id, req.body);
    res.status(201).json({ status: 'success', data });
});
exports.getJobApplicants = (0, error_1.catchAsync)(async (req, res) => {
    const data = await jobService.getJobApplicants(req.params.id);
    res.json({ status: 'success', data });
});
