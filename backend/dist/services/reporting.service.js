...
exports.getLeaveStats = exports.getDashboardStats = void 0;
...
const getDashboardStats = async (user) => {
    ...
    if (user.role.name === 'Admin' || user.role.name === 'Director' || user.role.name === 'HR') {
        ...
        const dept = await prisma_1.prisma.department.findFirst({ where: { managerId: user.id } });
        if (dept) {
            scope = {
        ...
        const team = await prisma_1.prisma.team.findUnique({ where: { leaderId: user.id } });
        if (team) {
    ...
    const activeLeaves = await prisma_1.prisma.leaveRequest.findMany({
        where: {
            userId: { in: employees.map(e => e.id) },
            status: 'APPROVED',
