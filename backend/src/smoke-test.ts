...
async function runTests() {
    ...
    if (health.status === 200) console.log('✅ Backend is UP');
    else console.error('❌ Backend is DOWN', health.data);
    ...
    if (dept.status === 201) console.log('✅ Department Created');
    else console.error('❌ Create Department Failed', dept.data);
    ...
    if (job.status === 201) console.log('✅ Job Created');
    else console.error('❌ Create Job Failed', job.data);
    ...
    if (punchIn.status === 201) console.log('✅ Punch In Successful');
    else console.error('❌ Punch In Failed', punchIn.data);
    ...
    if (project.status === 201) {
        ...
        if (task.status === 201) console.log('✅ Task Created');
        else console.error('❌ Create Task Failed', task.data);
    } else {
